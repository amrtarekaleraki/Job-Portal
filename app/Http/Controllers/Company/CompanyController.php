...
class CompanyController extends Controller
{
    ...
    public function photos()
    {
       ...
       if(!$order_data)
       {
        return redirect()->back()->with('error','Your have to choose a package first to access this page');
       }

        // check if a person has a access under the current package
       ...
       {
        return redirect()->back()->with('error','Your current package don`t allow you to upload photos');
       }
    ...
    public function photos_submit(Request $request)
    {
       ...
       $existing_photo_number =  CompanyPhoto::where('company_id',Auth::guard('company')->user()->id)->count();
        ...
        {
            return redirect()->back()->with('error','Maximum number of allowed photos is uploade, so you have to upgrade your package.');
        }
    ...
    public function videos()
    {
       ...
       {
        return redirect()->back()->with('error','Your current package don`t allow you to upload videos');
       }
    ...
    public function videos_submit(Request $request)
    {
       ...
       $existing_video_number =  CompanyVideo::where('company_id',Auth::guard('company')->user()->id)->count();
        ...
        {
            return redirect()->back()->with('error','Maximum number of allowed videos is uploade, so you have to upgrade your package.');
        }
    ...
    public function jobs_create()
    {
        ...
        {
            return redirect()->back()->with('error','Your current package don`t allow you to post jops');
        }


        // how many jobs the company posted
    ...
    public function jobs_create_submit(Request $request)
    {
            ...
            // how many featured jobs the company selected
             $total_featured_jobs = Job::where('company_id',Auth::guard('company')->user()->id)->where('is_featured',1)->count();
            ...
            if($total_featured_jobs == $package_data->total_allowed_featured_jobs)
            {
                ...
                {
                    return redirect()->back()->with('error','You already have added the total number of featurd jobs ');
                }
    ...
    {
         $jobs = Job::with('rJobCategory','rJobLocation','rJobType','rJobGender','rJobSalaryRange','rJobExperience')->where('company_id',Auth::guard('company')->user()->id)->get();
    ...
    {
       $applicants =  CandidateApply::with('rCandidate')->where('job_id',$id)->get();
    ...
    public function applicant_resume($id)
    {
        ...
        $candidate_experiencs = CandidateExperience::where('candidate_id',$id)->get();
        ...
        return view('company.applicant_resume',compact('candidate_single','candidate_educations','candidate_experiencs','candidate_skills','candidate_awards','candidate_resumes'));
    }
    ...
    public function application_status_change(Request $request)
    {
        ...
        $obj =  CandidateApply::with('rCandidate')->where('candidate_id',$request->candidate_id)->where('job_id',$request->job_id)->first();
        ...
        if($request->status == "Approved")
        {
            ...
            $subject = 'Congratulations your applications is approved';
